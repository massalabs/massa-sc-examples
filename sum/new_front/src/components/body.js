...
export default function Body() {
    ...
    function createAccount() {
        ...
        // create an account
        const url = MASSA_STATION_WALLET_URL + 'api/accounts/' + document.getElementsByClassName("input")[0].value;
        console.log(url);
            ...
            if (ok) {
                ...
                .finally(() => {
                    if (ok) {
                        ...
                        onMSDisconnected("The MassaStation wallet plugin is not running");
                    }
                }
                );
            } else {
                ...
                { connected && account !== 'No account found' &&
                    <div>
                        <div className='accountInfo'>
                            <br></br>
                            <p className="address">
                                Account : <a className='addressLink' href={"https://massa.net/testnet/"+account}>{account.substring(0,6) + ' ... ' + account.substring(account.length -5, account.length)}</a>
                            </p>
