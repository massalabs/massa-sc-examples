...
import { Args, bytesToStr, IClient, ClientFactory, strToBytes, fromMAS } from "@massalabs/massa-web3";
...
const CONTRACT_ADDRESS =
    "AS1oCgRnX5CTv4hiyfq5zoKuhU9xqL2nYRvzAULnHersTumPZHz4";
        ...
        return (
            <div className="relative bg-secondary mas-body flex flex-col justify-center items-center w-full max-w-5xl p-8 box-border rounded-lg shadow-md mb-12 mx-auto">
                ...
                {!showPublish && !selectedPost ? (
                    <div>
                        <button id="refreshButton" onClick={fetchPosts} className="refresh-button absolute top-4 right-4 bg-transparent border border-gray-400 px-2 py-1 rounded-md m-1">
                            Refresh Posts
                        </button>
                    ...
                    <div>
                        ...
                        <h2 id="selectedTitle" className="mt-2 text-xl font-bold divider" style={{ wordBreak: 'break-all' }} >{selectedPost.Title}</h2>
                        <div className="overflow-auto text-left">
                            <span className="text-right text-sm">By </span>
                            <h3 id="selectedAuthor" className="inline-block"> {selectedPost.Author} </h3>
                        </div>
                        <div className="flex flex-wrap mt-1 text-xs text-left w-full divider">
                            {selectedPost.Tags.split(',').map((tag, tagIndex) => (
                                <div id="selectedTag" key={tagIndex} className="tag bg-transparent border border-gray-400 px-2 py-1 rounded-md m-1">{tag.trim()}</div>
                            ))}
                        </div>
                        <p id="selectedContent" className="mt-2 overflow-auto text-left" style={{ wordBreak: 'break-all' }}>{selectedPost.Content}</p>
                    </div>
                ) : (
                    <div className="publish-section">
                        ...
                        <div>
                            <label>Title</label>
                            <input id="inputTitle" type="text" className="input" value={title} onChange={handleTitleChange} />
                        </div>
                        <div>
                            <label>Author</label>
                            <input id="inputAuthor" type="text" className="input" value={author} onChange={handleAuthorChange} />
                        </div>
                        <div>
                            <label>Tags</label>
                            <input id="inputTag" type="text" className="input" value={tags} onChange={handleTagsChange} />
                        </div>
                        <div>
                            <label>Content</label>
                            <textarea id="inputContent" className="input" value={content} onChange={handleContentChange} />
                        </div>
                        <button
                            id="submitButton"
                            className="button flex justify-center items-center border border-green-500 p-3 text-green-500 rounded-md hover:bg-green-500 hover:text-white"
