...
export function deletePost(binaryArgs: StaticArray<u8>): void {
    const argsDeser = new Args(binaryArgs);
    //Get the post index from the arguments and check if it is valid (not null), the expected format is a string of the form "1.0" or "2.0" or "3.0", etc. 
...
//This function is used within the contract to generate the key of a post that will be stored in the storage of the contract
